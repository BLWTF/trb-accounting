...
class JournalLedger extends Observer
{
    ...
    public function deleted(Model $ledger)
    {
        ...
        Transaction::type('income')->whereNull('document_id')->where('reference', 'journal-entry-ledger:' . $ledger->id)->delete();
        Transaction::type('expense')->whereNull('document_id')->where('reference', 'journal-entry-ledger:' . $ledger->id)->delete();
