...
class BillInput
{
    ...
    public function compose(View $view)
    {
        ...
        $additional_types = Type::where('class_id', 1)->whereIn('name', ['double-entry::types.fixed_asset', 'double-entry::types.inventory'])->pluck('name', 'id')->map(function ($name) {
            return trans($name);
        })->toArray();

        $types = array_replace($types, $additional_types);
